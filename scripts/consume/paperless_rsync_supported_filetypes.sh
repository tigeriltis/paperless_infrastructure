#!/bin/bash
if [ $# -le 1 ] ; then echo Give at least one file or folder to consume and the target directory as argument. ; exit ; fi
...
rsync -av --no-owner --no-group --exclude=~* --exclude=.git --include=*/ --include=\*.{pdf,jpeg,jpg,png,tif,gif,txt,rtf,odt,ods,odp,doc,docx,ppt,pptx,xls,xlsx,eml} --exclude=\* $@
